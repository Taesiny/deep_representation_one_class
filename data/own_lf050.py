...
class Owndata_lf050(CIFAR):
  """DogVsCat data loader."""

  def __init__(self, root, dataset='owndata_lf050', input_shape=(32, 32, 3)):
    assert input_shape[0] == input_shape[1]
    np_path = os.path.join(
        root, 'own_data_lf050', 'train_{}x{}.npz'.format(input_shape[0],
                                                       input_shape[1]))
...
class OwndataOOD_lf050(Owndata_lf050):
  """Owndata for OOD."""

  def __init__(self,
               root,
               dataset='owndata_lf050',
               input_shape=(32, 32, 3),
               category=0):
    super(OwndataOOD_lf050, self).__init__(
