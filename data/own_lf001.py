...
class Owndata_lf001(CIFAR):
  """DogVsCat data loader."""

  def __init__(self, root, dataset='owndata_lf001', input_shape=(32, 32, 3)):
    assert input_shape[0] == input_shape[1]
    np_path = os.path.join(
        root, 'own_data_lf001', 'train_{}x{}.npz'.format(input_shape[0],
                                                       input_shape[1]))
...
class OwndataOOD_lf001(Owndata_lf001):
  """Owndata for OOD."""

  def __init__(self,
               root,
               dataset='owndata_lf001',
               input_shape=(32, 32, 3),
               category=0):
    super(OwndataOOD_lf001, self).__init__(
