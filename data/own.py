...
class Owndata(CIFAR):
  ...
  def __init__(self, root, dataset='owndata', input_shape=(32, 32, 3)):
    assert input_shape[0] == input_shape[1]
    np_path = os.path.join(
        root, 'own_data', 'train_{}x{}.npz'.format(input_shape[0],
                                                       input_shape[1]))
    ...
    train_image_pos = image_pos[randidx_pos[:100]]
    train_label_pos = label_pos[randidx_pos[:100]]
    test_image_pos = image_pos[randidx_pos[100:]]
    test_label_pos = label_pos[randidx_pos[100:]]
    randidx_neg = np.random.permutation(image_neg.shape[0])
    train_image_neg = image_neg[randidx_neg[:100]]
    train_label_neg = label_neg[randidx_neg[:100]]
    test_image_neg = image_neg[randidx_neg[100:]]
    test_label_neg = label_neg[randidx_neg[100:]]
...
class OwndataOOD(Owndata):
  ...
  def __init__(self,
               ...
               input_shape=(32, 32, 3),
               category=0):
    super(OwndataOOD, self).__init__(
