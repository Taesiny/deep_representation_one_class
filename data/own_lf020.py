...
class Owndata_lf020(CIFAR):
  """DogVsCat data loader."""

  def __init__(self, root, dataset='owndata_lf020', input_shape=(32, 32, 3)):
    assert input_shape[0] == input_shape[1]
    np_path = os.path.join(
        root, 'own_data_lf020', 'train_{}x{}.npz'.format(input_shape[0],
                                                       input_shape[1]))
...
class OwndataOOD_lf020(Owndata_lf020):
  """Owndata for OOD."""

  def __init__(self,
               root,
               dataset='owndata_lf020',
               input_shape=(32, 32, 3),
               category=0):
    super(OwndataOOD_lf020, self).__init__(
